  ...
  <div class="inline-calendar" :class="{'is-weekend-highlight': highlightWeekend}">
    ...
    <table>
      <thead v-show="!hideWeekList">
        <tr style="background-color:#ef4d65;color:white;line-height:2.5rem;">
      ...
      <tbody>
        <tr v-for="(k1,day) in days">
          ...
          :class="buildClass(k2, child, choice(formatDate(year, month, child)) && !child.isLastMonth && !child.isNextMonth)"
  ...
  methods: {
    ...
    buildClass (index, child, isCurrent) {
      const className = {
        ...
        'is-today': child.isToday && !child.isNextMonth && !child.isLastMonth
      }
      className[`is-week-${index}`] = true
//      if(child.isToday) alert(JSON.stringify(child));
      return className
    },
    //新增接口
    choice(data){
      let flag =false;
      this.valueLst && this.valueLst.forEach((item)=>{
       if( item.sign_time.substr(0,10) == data) flag = true;
      })
      return flag;
    },
    render (year, month) {
